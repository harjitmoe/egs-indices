...
def extract_date2id():
    ...
    def getdate(fn):
        return fn[3:][:10], (int(fn[pl:][:5])
                             if fn[pl:][:5].strip("xn") else None)

    stmap = dict(
        list(
            map(getdate, [
                i for i in lsdir
                if i.startswith("ST-") and not i.lower().count("-original")
    ...
    npmap = dict(
        list(
            map(getdate, [
                i for i in lsdir
                if i.startswith("NP-") and not i.lower().count("-original")
    ...
    sbmap = dict(
        list(
            map(getdate, [
                i for i in lsdir
                if i.startswith("SB-") and not i.lower().count("-original")
                and not i.lower().count("-xxxxx")
            ])))
    if stmap and npmap and sbmap:
        open("Date2Id.txt", "w").write(
            json.dumps({
                "story": stmap,
                "sketch": sbmap,
                "np": npmap
