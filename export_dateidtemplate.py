...
def export_dateidtemplate(alldat):
    ...
    for outf, db, title in ((ditst, stdb,
                             "Structural Nomenclature|Main Story"),
                            (ditsb, sbdb, "Sketchbook"), (ditnp, npdb,
                                                          "EGS:NP")):
        print("<includeonly>{{#switch:{{{1}}}", file=outf)
        for arc in db:
            for line in (arc["StoryLines"]
                ...
                for comic in line["Comics"]:
                    ...
                    print("|%s=%d" % (date, comic["Id"]), file=outf)
                    if comic['SharedDateIndex']:
                        print("|%s of %s=%d" %
                              (date, (comic['SharedDateTotal'][0]
                                      if 'SharedDateTotal' in comic else -1),
                               comic["Id"]),
                              file=outf)
        print("}}</includeonly>" + (docs % title), file=outf)
        outf.close()
